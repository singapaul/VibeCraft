...
export async function getSongs({prompt, count, user}: {prompt: string, count: number, user: SpotifyApi.CurrentUsersProfileResponse}) {
...
export async function searchSongs({playlist, user, prompt}: {playlist: SongArtist[], user: SpotifyApi.CurrentUsersProfileResponse | undefined, prompt:string}) {
